#!/usr/bin/env python3
"""cocotb tests for dec_sse_bin / dec_bin_generic (M7).
...
# Default SSE TB offsets (same §6 synthetic layout)
OFF_TYPE = 0
OFF_SEQ = 4
OFF_CODE = 8
OFF_PX = 14
...
async def _check_msg(dut, msg_type: int, expect_ch: int):
    ...
    # m_code sideband: ASCII "000001", char0 in [7:0]
    code_exp = 0
    for i, ch in enumerate(b"000001"):
        code_exp |= ch << (8 * i)
    assert int(dut.m_code.value) == code_exp, hex(int(dut.m_code.value))
...
async def test_order_msg(dut):
    """ORDER msg_type=2 → CH_ORDER, fields match, exch=SSE."""
    await _init(dut)
